...
const schema = require('../validations/alert');
      ...
      .then(msg => {
        ...
        .then(() => {
          ...
          for (let i = 0; i < recipientArray.length; i++) {
            ...
            for (let index = 0; index < recipientPhoneArray.length; index++) {
              if(recipientPhoneArray[index].length < 12){
                recipientPhoneArray[index] = recipientPhoneArray[index].toString().replace(/0/, '+234') 
              }
              Sms.send(recipientPhoneArray[index], message, reqId, logData);
            }
          }
